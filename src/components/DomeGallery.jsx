...
export default function DomeGallery({
  ...
  minRadius = 600,
  maxRadius = Infinity,
  padFactor = 0.05, // Modificato: rimpicciolito il pad per lasciare spazio all'immagine grande
  ...
  dragDampening = 2,
  openedImageWidth = '80vw', // Modificato: impostata larghezza grande (80% della vista)
  openedImageHeight = '40vh', // Modificato: impostata altezza grande (80% della vista)
  ...
  useEffect(() => {
    return () => {
      ...
      const overlays = document.querySelectorAll('.enlarge, .enlarge-closing, .item__image--reference');
      overlays.forEach(el => el.remove());

      // 2. Ripristiniamo la visibilità di eventuali icone nascoste
      const hiddenImages = document.querySelectorAll('.item__image');
      hiddenImages.forEach(img => {
        img.style.visibility = 'visible';
        img.style.opacity = '1';
      });

      // 3. Sblocchiamo lo scroll
    ...
    const ro = new ResizeObserver(entries => {
      ...
      if (enlargedOverlay && frameRef.current && mainRef.current) {
        ...
        if (hasCustomSize) {
          ...
          enlargedOverlay.style.width = `${tempRect.width}px`; // Aggiunto per forzare larghezza
          enlargedOverlay.style.height = `${tempRect.height}px`; // Aggiunto per forzare altezza
        } else {
...
const startInertia = useCallback((vx, vy) => {
  ...
  const cssStyles = `
    .sphere-root {
      --radius: 520px;
      --viewer-pad: 20px; /* Ridotto pad per permettere immagine più grande */
